...
We perform posterior predictive inference for the largest value $M_N$ to be observed over a future time period of length $N$ years.  Objects returned from the `blite` function have a `predict` method based on the `predict.evpost` method in the @revdbayes package. The effect of adjusting for the values of the extremal index \eqn{\theta} in the posterior sample is to change the effective time horizon from $N$ to $\theta N$. See the Posterior predictive extreme value inference section of the [Posterior Predictive Extreme Value Inference using the revdbayes Package](https://cran.r-project.org/web/packages/revdbayes/vignettes/revdbayes-c-predictive-vignette.html) vignette.
